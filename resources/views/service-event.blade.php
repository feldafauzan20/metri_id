...
<x-layouts.layout>
    ...
    <main>
        ...
        {{-- HERO START --}}
        <section aria-label="Main Section: Event Planning and Execution">
                    ...
                    <div class="w-full flex flex-col items-center">
                        ...
                        <div class="w-full justify-center text-base flex text-white py-6 font-ibm font-medium">
                            ...
                            <a wire:navigate href="#video"
                                ...
                                aria-label="Scroll to Event Showcase Video">
        ...
        {{-- Video start --}}
        <section class="w-full" id="video" aria-label="Event Showcase Video Section">
                ...
                @if (is_array($video))
                    @foreach ($video as $vid)
                        ...
                        <video class="object-cover w-full h-full" autoplay loop playsinline muted
                            title="Metri Event Execution Video">
            ...
            @else
                ...
                <p class="text-center text-gray-400">No event showcase video available.</p>
            @endif
        </section>
        {{-- Video end --}}

        {{-- PORTO START --}}
        <section class="lg:mx-4" id="portfolio" aria-label="Event Portfolio Section">
            <div class="grid lg:grid-cols-4 md:grid-cols-2 gap-4 lg:mx-auto mx-4 lg:py-20 py-10">
                <img src="{{ asset('storage/' . $photo_1) }}" alt="Event Portfolio: Project 1"
                    class="col-span-1 row-span-2 w-full h-full object-cover rounded-lg" loading="lazy">

                <img src="{{ asset('storage/' . $photo_2) }}" alt="Event Portfolio: Project 2"
                    class="col-span-1 row-span-1 w-full h-full object-cover rounded-lg" loading="lazy">

                <img src="{{ asset('storage/' . $photo_3) }}" alt="Event Portfolio: Project 3"
                    class="col-span-1 row-span-2 w-full h-full object-cover rounded-lg" loading="lazy">

                <img src="{{ asset('storage/' . $photo_4) }}" alt="Event Portfolio: Project 4"
                    class="col-span-1 row-span-2 w-full h-full object-cover rounded-lg" loading="lazy">

                <img src="{{ asset('storage/' . $photo_5) }}" alt="Event Portfolio: Project 5"
                    class="col-span-1 row-span-2 w-full h-full object-cover rounded-lg" loading="lazy">
            </div>
        </section>
        {{-- PORTO END --}}
        ...
        {{-- PROJECT START --}}
        <section class="my-32 px-4" id="proyek" aria-label="Recent Event Projects Section">
            <div class="w-full px-4 md:px-8 lg:flex lg:justify-end">
                <a wire:navigate href="/gallery" target="_self" class="flex items-center space-x-2 group text-white"
                    aria-label="View More Event Projects">
            ...
            <div class="grid grid-cols-1 gap-6 mx-4 md:mx-8 mt-8 md:grid-cols-2 lg:grid-cols-3">
                @forelse ($projects as $project)
                    <div class="relative overflow-hidden rounded-lg h-96 group"
                        aria-label="Event Project: {{ $project->title }}">
                        ...
                        <div
                            ...
                            <div
                                ...
                                <a wire:navigate href="{{ route('detail', ['slug' => $project->slug]) }}"
                                    ...
                                    aria-label="View Event Project: {{ $project->title }}">
                                    VIEW PROJECT
                                </a>
                            </div>
                        </div>
                ...
                @empty
                    <p class="text-white text-center col-span-3">No event projects available.</p>
                @endforelse
            </div>
        </section>
        {{-- PROJECT END --}}
